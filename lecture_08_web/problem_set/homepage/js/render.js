export function renderCursor(caretElement, cursorState, words) {
  ...
  if (!currentLetterElement) {
    const previousLetterElement =
      currentWord.children[cursorState.charIndex - 1];
...
export function renderOnDeleteWord(cursorState, words, inputElement) {
  ...
  const lastWordLength =
    inputElement.value.length - currentWord.children.length;
...
export function generateNewWords(
  data,
  wordsAmount,
  cursorState,
  words,
